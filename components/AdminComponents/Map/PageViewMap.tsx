...
function hexToRgba(hex: string, alpha: number): string {
  ...
  if (h.length !== 3 && h.length !== 6) return `rgba(245, 158, 11,${alpha})`;
...
export default function PageViewMap({ data, height = 380 }: PageViewMapProps) {
  ...
  useEffect(() => {
    ...
    const accent = (cs?.getPropertyValue('--admin-rail').trim() || '#f59e0b');
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          borderLeft: '3px solid var(--admin-rail, #f59e0b)',
          borderRadius: '2px',
