...
import {
  ...
  faHouseChimney, faHammer, faBath, faBuilding, faDraftingCompass,
  faClipboardList, faHardHat,
...
const SERVICES = [
  { icon: faHouseChimney,    label: 'New Home Construction' },
  { icon: faHammer,          label: 'Additions & Remodels' },
  { icon: faBath,            label: 'Kitchen & Bath' },
  { icon: faBuilding,        label: 'Commercial' },
  { icon: faDraftingCompass, label: 'Design-Build' },
  { icon: faClipboardList,   label: 'General Contracting' },
  { icon: faHardHat,         label: 'Other Project' },
];
          ...
          <div className={styles.leftPanel}>
            <div className={styles.panelInner}>
              ...
              <p className={styles.quoteText}>
                "Fair bid. Show up on time. Build it right. That's all anyone ever wants — and that's exactly what we do."
              </p>
              <span className={styles.quoteAttrib}>— Bedrock Construction Group, Waco TX</span>

              <div className={styles.panelStats}>
                {[
                  { val: '180+',  lbl: 'Projects Completed' },
                  { val: '4.8★',  lbl: 'Google Rating' },
                  { val: '14+',   lbl: 'Years Local' },
                ].map(s => (
              ...
              <div className={styles.panelBadges}>
                {['Licensed & Bonded', '2-Yr Warranty', 'Free Estimates', 'AGC Member'].map(b => (
              ...
              {submitted ? (
                <motion.div key="success" className={styles.successState}
                  ...
                  <p className={styles.successText}>
                    A Bedrock project manager will contact you shortly about your{' '}
                    <strong>{selectedServices.join(' & ')}</strong> project.
                    {urgent && " We saw you marked this as a rush timeline — we're on it."}
                  </p>
                </motion.div>
              ) : (
                <motion.div key="form" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
                  ...
                  <div className={styles.serviceSection}>
                    <p className={styles.serviceHeading}>What kind of project? <span>(select all that apply)</span></p>
                  ...
                  <div className={styles.urgencyRow}>
                    <button type="button"
                      ...
                      {urgent ? "🔴 Marked as rush — we'll prioritize your call" : 'Mark as rush / tight timeline'}
                    </button>
                  </div>
                  ...
                  <form className={styles.form} onSubmit={handleSubmit} noValidate>
                    ...
                    <div className={styles.field}>
                      <label className={styles.label} htmlFor="v2r-message">Describe your project</label>
                      <textarea id="v2r-message" className={`${styles.input} ${styles.textarea}`}
                        name="message" placeholder="What are you looking to build or renovate?" rows={3}
